...
SELECT 	S.ProjectNumber AS ProjectNumber,
		...
		IIF(S.CarryIn = 0, '2022', 'Carry-In') AS [CarryIn],
...
FROM forecast.Project AS P INNER JOIN forecast.Subproject AS S ON P.ProjectNumber = S.ProjectNumber
		...
		LEFT JOIN forecast.GrossAddsDirect AS GD ON S.ProjectNumber = GD.ProjectNumber AND S.SubprojectNumber = GD.SubprojectNumber AND GD.[Year] = 2022
		LEFT JOIN forecast.GrossAddsIndirect AS GI ON S.ProjectNumber = GI.ProjectNumber AND S.SubprojectNumber = GI.SubprojectNumber AND GI.[Year] = 2022
...
UPDATE forecast.ForecastExport
SET CarryIn = IIF(E.[AP Code]='','Multi-yr',IIF((E.[Bdgt Yr] + E.[Proj Life (Yrs)] -1)>=22,IIF(((E.[Bdgt Yr] = 22) AND (E.[Proj Life (Yrs)] <= 1)),'2022','Multi-yr'),'Carry-In'))
