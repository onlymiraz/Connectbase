CREATE VIEW [TOWERS_UNIVERSE].[BI_Dashboard]
	-- AS SELECT * FROM [SomeTableOrView]
	AS SELECT
    CAST(MASTER_TOWER_ID AS BIGINT) AS 'Master Tower ID',
    CASE 
    ...
    CAST(CENSUS_BLOCK_CODE_2019 AS BIGINT) AS 'Census Block Code 2019',
    CAST(CENSUS_BLOCK_CODE_2020 AS BIGINT) AS 'Census Block Code 2020',
    ...
    TOTAL_NEWFIBEROSP_COSTS AS 'Total New Fiber OSP Costs',
    SPLICING_COST AS 'Splicing Cost',
    GUARDRAIL_COST AS 'Guardrail Cost',
    NID_SFP_COSTS_BELOW_1G AS 'NID SFP Costs Below 1G',
    NID_SFP_COSTS_1G_TO_10G AS 'NID SFP Costs 1G to 10G',
    NID_SFP_COSTS_ABOVE_10G AS 'NID SFP Costs Above 10G',
    TR_COSTS_BELOW_1G AS 'TR Costs Below 1G',
    TR_COSTS_1G_TO_10G AS 'TR Costs 1G to 10G',
    TR_COSTS_ABOVE_10G AS 'TR Costs Above 10G',
    SERVABLE AS Servable,
    REVIEW_STATUS AS 'Review Status',
    REVIEW_REASON AS 'Review Reason',
    ROUTE_NOTES AS 'Route Notes'
FROM TOWERS_UNIVERSE.TBL_TOWER_MASTER;