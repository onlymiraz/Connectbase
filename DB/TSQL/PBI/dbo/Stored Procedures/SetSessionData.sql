...
FROM [PowerBIReportServerTempDB].dbo.SessionData WITH (XLOCK)
...
BEGIN -- Yes, update it
   ...
   UPDATE
      [PowerBIReportServerTempDB].dbo.SessionData
   SET
   ...
   END ELSE BEGIN
      UPDATE
         [PowerBIReportServerTempDB].dbo.SnapshotData
      SET
         ExpirationDate = DATEADD(n, @SnapshotTimeoutSeconds, @now)
      WHERE
         SnapshotDataID = @SnapshotDataID
...
BEGIN -- no, insert it
   UPDATE PS
    ...
    INSERT INTO [PowerBIReportServerTempDB].dbo.SessionData
