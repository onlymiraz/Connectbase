...
FROM
    [PowerBIReportServerTempDB].dbo.SessionData AS SE WITH (XLOCK)
WHERE
...
FROM
    [PowerBIReportServerTempDB].dbo.SessionData AS SE
    INNER JOIN [PowerBIReportServerTempDB].dbo.SnapshotData AS SN ON SN.SnapshotDataID = SE.SnapshotDataID
    LEFT OUTER JOIN [PowerBIReportServerTempDB].dbo.SnapshotData AS COMP ON SE.CompiledDefinition = COMP.SnapshotDataID
WHERE
   SE.SessionID = @DBSessionID
