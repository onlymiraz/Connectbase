...
IF @FromIsPermanent != 0 AND @ToIsPermanent = 0 BEGIN
    ...
    INSERT INTO [PowerBIReportServerTempDB].dbo.ChunkData
    ...
    NOT EXISTS(
        SELECT T.ChunkName
        FROM [PowerBIReportServerTempDB].dbo.ChunkData AS T -- exclude the ones in the target
        WHERE
    ...
    select @Machine = Machine from [PowerBIReportServerTempDB].dbo.SnapshotData SD where SD.SnapshotDataID = @ToSnapshotID

    INSERT INTO [PowerBIReportServerTempDB].dbo.SegmentedChunk
    ...
    WHERE
        ...
        NOT EXISTS(
            ...
            FROM [PowerBIReportServerTempDB].dbo.SegmentedChunk TSC
            -- JOIN [PowerBIReportServerTempDB].dbo.SnapshotChunkMapping TSCM ON (TSC.ChunkId = TSCM.ChunkId)
            WHERE
 ...
 END ELSE IF @FromIsPermanent = 0 AND @ToIsPermanent = 0 BEGIN
    ...
    select @Machine = Machine from [PowerBIReportServerTempDB].dbo.SnapshotData SD where SD.SnapshotDataID = @FromSnapshotID
    ...
    SELECT
        newid(), @ToSnapshotID, COALESCE(@TargetChunkName, S.ChunkName), S.ChunkType, S.MimeType, S.Version, S.ChunkFlags, S.Content
    FROM
        [PowerBIReportServerTempDB].dbo.ChunkData AS S
    WHERE
        S.SnapshotDataID = @FromSnapshotID AND
        (S.ChunkType = @ChunkType OR @ChunkType IS NULL) AND
        (S.ChunkName = @ChunkName OR @ChunkName IS NULL) AND
    ...
    FROM [PowerBIReportServerTempDB].dbo.SegmentedChunk C WITH(INDEX (UNIQ_SnapshotChunkMapping))
    WHERE	C.SnapshotDataId = @FromSnapshotID AND
            ...
            NOT EXISTS(
                ...
                SELECT T.ChunkId
                FROM [PowerBIReportServerTempDB].dbo.SegmentedChunk T
...
END ELSE IF @FromIsPermanent = 0 AND @ToIsPermanent != 0 BEGIN
    ...
    WHERE
     ...
     FROM [PowerBIReportServerTempDB].dbo.ChunkSegmentMapping CM
     ...
     FROM [PowerBIReportServerTempDB].dbo.Segment CS
