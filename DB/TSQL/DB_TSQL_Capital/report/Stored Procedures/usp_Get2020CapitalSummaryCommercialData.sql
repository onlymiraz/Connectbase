...
create procedure report.usp_Get2020CapitalSummaryCommercialData
as
	set statistics io, time on;
	set nocount on;
begin try
	begin transaction
select GAMTSK 'Prime', GASTSK 'Sub', GAMTRX 'Cost Code', GAJTCD 'Just Code', ROUND(SUM(GARPT$),2) 'Spent $', GAACTY 'Year', GAACTP 'Month',
			Category 'Category', Title 'Title', [STATE] 'State', c.dir_indir 'Direct/Indirect', CONCAT(Title, ' ', c.dir_indir) 'Title Lookup',
			GAPRJ# 'Proj #', GAPRJS 'Sub #', GAFUNC 'Func Group', GAREF 'GA Link Code'
			, A.LinkCode 'Criteria Link Code', A.LinkCodeFrom 'LC Link Code From'
from (select P.ProjectNumber, 
	...
	where CRI.BudgetCategoryID is not null and ((CriteriaID >= 66 and CriteriaID <= 89) or CriteriaID in (90, 91, 92))
	union
	select null,
			CAT.Category,
			CRI.CriteriaID, CRI.Title, CRI.InJustificationCode, CRI.InState, CRI.BudgetCategoryID, CRI.SubBudgetCategoryID, CRI.LinkCode, CRI.InFunctionalGroup,
	...
	from report.CapitalSummary2020Criteria CRI
		...
		left join report.CapitalSummary2020InAccount ACC on ACC.AccountID = CRI.InAccountID
		left join report.CapitalSummary2020LinkCodeMap LC on LC.LinkCodeTo = CRI.LinkCode
	where CRI.BudgetCategoryID is null and ((CriteriaID >= 66 and CriteriaID <= 89) or CriteriaID in (90, 91, 92))) A
...
where GAPRJS in (1,4) and GAACTP < 10 and GAJTCD in (66, 67, 68, 73, 78)
