...
FROM [LOG].[Tracker]
		...
		WHEN MATCHED THEN
			...
			SET S.[BudgetLineNumber] = F.[BudgetLineNumber],
				...
				S.[CarryIn] = IIF(F.[CarryIn] = '2024', 0, 1),
		...
		WHEN NOT MATCHED THEN
			...
			VALUES (F.ProjectNumber, F.SubprojectNumber, F.BudgetLineNumber, F.ProjectStatusCode, F.ApprovalDate, F.EstimatedStartDate, F.EstimatedCompleteDate,
					F.ActualStartDate, F.ReadyForServiceDate, F.TentativeCloseDate, F.CloseDate, F.SubprojectStatus, IIF(F.[CarryIn] = '2024', 0, 1), F.SentToClosing);
