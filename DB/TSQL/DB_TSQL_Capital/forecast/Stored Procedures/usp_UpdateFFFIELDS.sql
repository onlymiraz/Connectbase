...
FROM [LOG].[Tracker]
		...
		DECLARE @curYr nvarchar(4) = CAST(YEAR(GETDATE()) as nvarchar(4))
		DECLARE @curDate nvarchar(8)=@curYr+'0101'
		--select @curDate
		...
		SET	S.[BudgetLineNumber] = [@t].[BudgetLineNumber], 
			...
			S.[CarryIn] = IIF(S.[CarryIn] IS NOT NULL, S.[CarryIn], IIF([@t].[ApprovalDate] IS NULL, IIF([@t].[ProjectStatusCode] <> 'SP', 0, 1), IIF([@t].[ApprovalDate] > @curDate, 0, 1)))
