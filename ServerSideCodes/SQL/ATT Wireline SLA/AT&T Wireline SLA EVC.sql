...
PNUM, ETHERNET_READY_DT, LAST_ORDER_NO, LAST_COMP_DT, ACT, EVC_BDW, EVC_LEVEL,
PRODUCT, EVC_DESIGN_ID, EVC_CKT, UNI_CKT, NNI_CKT, UNI_DESIGN_ID, NNI_DESIGN_ID
FROM (
-- 8 
SELECT BAN, ACNA, 
REPLACE(CUSTOMER,',') AS CUSTOMER, ADDRESS, TRIM(CITY) CITY, STATE, LATA, PNUM, 
...
CASE WHEN SUBSTR(PNUM,1,3) = 'EPA' THEN 'EPATH'
     WHEN SUBSTR(PNUM,1,3) IN ('EVP','PLA','GLD') THEN 'EVPL'
...
NULL Z_ADD, NULL Z_CITY, NULL Z_STATE,
NID_TYPE, C5.LOCATION_ID_2, 
...
-- 7 
SELECT BAN, ACNA, CUSTOMER, ADDRESS, CITY, STATE, LATA,
PNUM, DD_COMP, DOCUMENT_NUMBER, ACT, EVC_BDW, LOS, EVC_SPEC, EVC_LEVEL,
...
ADD2.ADDR_LN1 ADDRESS, GA2.INSTANCE_VALUE CITY, GA.INSTANCE_VALUE_ABBREV STATE,
...
SELECT EVC_CKT, EVC_DESIGN_ID, ACNA, BAN, A.DOCUMENT_NUMBER, EVC_IND, ACT, DD_COMP, UNI_SPEC, PNUM, LOS, EVC_SPEC,
       LATA, EVC_BDW, EVC_LEVEL, EVC_STATUS, EVC_STATUS_DESC,
...
SELECT DISTINCT CKT, ACNA, DOCUMENT_NUMBER, BAN, EVC_IND, ACT, DD_COMP, UNI_SPEC, PNUM, LOS, EVC_SPEC,
       ...
       CASE WHEN LOS = ('EBE') THEN 'Silver'
            ...
            WHEN EVC_SPEC = 'EPATHAS' THEN 'Silver'
            WHEN EVC_SPEC = 'EPATHAG' THEN 'Gold'
            WHEN EVC_SPEC = 'EPATHAP' THEN 'Platinum'
            WHEN EVC_SPEC = 'EPATHSS' THEN 'Silver'
            WHEN EVC_SPEC = 'EPATHSG' THEN 'Gold'
            WHEN EVC_SPEC = 'EPATHSP' THEN 'Platinum'
...
SELECT DISTINCT C.EXCHANGE_CARRIER_CIRCUIT_ID CKT, SR.ACNA, SR.DOCUMENT_NUMBER, EVC_IND, TRUNC(ACTUAL_COMPLETION_DATE) DD_COMP,
       ...
       ASR.LATA_NUMBER LATA, CO.LATA_NUMBER, ELS.LVL_OF_SERV_NM AS LOS, ELS.SERVICE_AND_PRODUCT_ENHANC_COD AS EVC_SPEC, 
       ELS.BANDWIDTH BDW, ELS.BANDWIDTH_UOM UOM, AUD.BAN, C2.STATUS EVC_STATUS, C.CIRCUIT_DESIGN_ID,
       DECODE (C2.STATUS,
                  '1', 'Pending',
                  '2', 'Assigned',
                  '3', 'In Progress',
...
WHERE SR.DOCUMENT_NUMBER = ASR.DOCUMENT_NUMBER
  ...
  AND TO_CHAR(T.ACTUAL_COMPLETION_DATE,'yyyymm') = '202311'     --CHANGE DATE TO SHOW THE PREVIOUS MONTH   
  AND SUBSTR(PROMOTION_NBR,1,3) IN ('EVP','PLA','GLD','EPA')
...
-- 6
  ...
  GA_INSTANCE GA,
  GA_INSTANCE GA2
WHERE B.UNI_DESIGN_ID = CIRC.CIRCUIT_DESIGN_ID
  AND CIRC.LOCATION_ID = NLA2.LOCATION_ID (+)
  AND NLA2.ADDRESS_ID = ADD2.ADDRESS_ID (+)
  AND ADD2.GA_INSTANCE_ID_STATE_CD = GA.GA_INSTANCE_ID (+)
  AND ADD2.GA_INSTANCE_ID_CITY = GA2.GA_INSTANCE_ID (+) 
-- 7 
)
-- 8 
)
...
-- 9 
  ...
  MV_GA_INSTANCE GA1,
  MV_GA_INSTANCE GA2
WHERE A3.LOCATION_ID_2 = NLA2.LOCATION_ID (+)
AND NLA2.ADDRESS_ID = ADDR.ADDRESS_ID (+)
...
WHERE SR.DOCUMENT_NUMBER = ASR.DOCUMENT_NUMBER
  ...
  AND TO_CHAR(T.ACTUAL_COMPLETION_DATE,'yyyymmdd') >= ('202308')      --CHANGE DATE TO SHOW THE PREVIOUS MONTH     
  AND ASR.REQUEST_TYPE IN ('S','E')
  AND ASR.ACTIVITY_INDICATOR IN ('D')
  AND ASR.ORDER_TYPE = 'ASR' 
  AND T.TASK_TYPE = 'DD'
  AND SR.ACNA NOT IN ('FLR','ZZZ','CUS','ZWV','ZTK')
