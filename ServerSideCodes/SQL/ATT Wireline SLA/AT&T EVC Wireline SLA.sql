...
ETHERNET_READY_DT, LAST_ORDER_NO, LAST_COMP_DT,  ACT, EVC_BDW, EVC_LEVEL, PRODUCT,
...
CASE WHEN ICSC IN ('GT10','GT11') AND STATE IN ('CA','FL','TX') THEN 'Carrier CTF' ELSE 'Carrier FTTC' END PRODUCT,
...
LATA, ICSC, PNUM,  DOCUMENT_NUMBER, DD_COMP, ACT, EVC_BDW, LOS, EVC_SPEC, EVC_LEVEL, EVC_CKR,
...
SELECT DISTINCT C.EXCHANGE_CARRIER_CIRCUIT_ID CKT, SR.ACNA, SR.DOCUMENT_NUMBER, EVC_IND, TRUNC(ACTUAL_COMPLETION_DATE) DD_COMP,
       ...
       ED.EVC_CKR, AUD.BAN, C2.STATUS EVC_STATUS, C.CIRCUIT_DESIGN_ID, ACCESS_PROVIDER_SERV_CTR_CODE ICSC,
       DECODE (C2.STATUS,
                  '1', 'Pending',
                  '2', 'Assigned',
                  '3', 'In Progress',
...
WHERE SR.DOCUMENT_NUMBER = ASR.DOCUMENT_NUMBER
  ...
  --AND SR.DOCUMENT_NUMBER in ('2804334')
  AND TO_CHAR(T.ACTUAL_COMPLETION_DATE,'yyyymmdd') >= '20220213'     --CHANGE DATE TO SHOW THE PREVIOUS SUNDAY  
...
--TO USE LOCATION ID TO PULL IN ADDRESS  
SELECT NLA.LOCATION_ID, REPLACE(ADDR.ADDR_LN1,'MSAG ') AS Z_ADDRESS, TRIM(GA1.INSTANCE_VALUE) Z_CITY, 
GA2.INSTANCE_VALUE_ABBREV Z_STATE
FROM
(SELECT DISTINCT *
        FROM (SELECT D.LOCATION_ID,
  ...
  MV_GA_INSTANCE GA1,
  MV_GA_INSTANCE GA2
WHERE NLA.ADDRESS_ID = ADDR.ADDRESS_ID
AND ADDR.GA_INSTANCE_ID_CITY = GA1.GA_INSTANCE_ID
AND ADDR.GA_INSTANCE_ID_STATE_CD = GA2.GA_INSTANCE_ID
AND NLA.LOCATION_ID = '382062'
ORDER BY 1
;