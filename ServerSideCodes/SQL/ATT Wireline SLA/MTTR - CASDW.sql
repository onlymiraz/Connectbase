...
select DISTINCT TICKET_ID, STATE, CLEC_ID, 
       REPLACE(REPLACE(CKT_ID,'/'),' ') CLEAN,
       CKT_ID, PRODUCT, PROD2, BDW, CREATE_DATE,
       ...
       case when request_type = 'Information' then 'No'
            when cause_cd = 'Frontier Fiber Cut (Not Frontier Caused)' then 'No'
            when DISP IN ('CO','FAC') THEN 'Yes'
            else 'No' end COUNTABLE,
...
select ticket_id, state, clec_id, cl.customer, ckt_id, product,
	   ...
	   case WHEN STATE IN ('NY','PA','CT','OH','WV','VA','MD') THEN 'East'
			...
			cause_cd, company, bdw, fault, plant, request_type
from (
select distinct ticket_id, state, clec_id, ckt_id, product, 
       ...
       repair_code, service_type_code, cause_cd, fault, plant, request_type
from (
select distinct ticket_id, 
	   ...
	   site_id, a.repair_code, service_type_code, cause_cd, fault, plant, request_type
from (
...
where a.fld_troublereportstate = 'closed'
 ...
 and (to_char(dte_closeddatetime,'yyyymm') in ('202311')    --NEED TO CHANGE THIS EACH MONTH
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202311')))   --NEED TO CHANGE THIS EACH MONTH
 and e.status (+) = '6'
...
and product not in ('DS1','DS3','OCN','DS0')
)					 
