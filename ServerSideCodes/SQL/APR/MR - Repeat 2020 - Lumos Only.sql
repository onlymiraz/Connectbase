...
drop table aprmrlumos
/

CREATE TABLE aprmrlumos NOLOGGING NOCACHE AS
select region, TO_CHAR(CLOSED_DT,'YYYYMM') MONTH, ticket_id, CUSTOMER, CKT_ID, product, STATE, CLEC_ID, create_date, cleared_dt, closed_dt,
       ...
       CASE WHEN DISP IN ('CO','FAC') THEN 'Y' ELSE 'N' END  FFDISP,
       CASE WHEN SUBSTR(CKT_ID,7,1) = 'U' THEN 'Y' ELSE 'N' END UNE    
from (
select ticket_id, state, clec_id, cl.customer, ckt_id, product, create_date, cleared_dt, closed_dt,  
       ttr, total_duration, trbl_found_cd, trbl_found_desc, disp, site_id, repair_code, serv_code,
...
select distinct ticket_id, 
       case when icsc = 'RT01' then 'NY'
	        when substr(circuit,1,2) = 'R2' then 'NY'
	         ...
	         and site_state in ('NY','PA','WV','MD','VA','SC','NC','IL','MN','OH','IN','KY','MI','CA','OR','WA','AZ','NV','NM','UT','WI','ID','IA','MT','NE','AL','FL','GA','MS','TN','TX') then site_state
            when priloc is not null 
			 and priloc in ('NY','PA','WV','MD','VA','SC','NC','IL','MN','OH','IN','KY','MI','CA','OR','WA','AZ','NV','NM','UT','WI','ID','IA','MT','NE','AL','FL','GA','MS','TN','TX') then priloc
...
where state in ('NY','PA','WV','MD','VA','SC','NC','IL','MN','OH','IN','KY','MI','CA','OR','WA','AZ','NV','NM','UT','WI','ID','IA','MT','NE','AL','FL','GA','MS','TN','TX')
and clec_id in ('FET','FWN')		 
)
...
SELECT TICKET_ID, CUSTOMER, CKT_ID, PRODUCT SERVICE, STATE, CLEC_ID ACNA, CREATE_DATE, CLEARED_DT CLEARED_DATE,
       CLOSED_DT CLOSE_DATE, REPAIR_CODE, DISP TRBL_GROUP, 
       case when UNE = 'Y' then null else round(TTR,2) end TTR
FROM aprmrlumos
WHERE CUSTOMER = 'LUMOS'
AND MONTH = '202312'	
ORDER BY CLOSED_DT;
...
drop table lumrepeat
/

create table lumrepeat nologging nocache as
select * 
from aprmrlumos
where to_char(closed_dt,'MM')  = '12'    --NEED TO CHANGE THIS EACH MONTH
and disp in ('CO','FAC');
