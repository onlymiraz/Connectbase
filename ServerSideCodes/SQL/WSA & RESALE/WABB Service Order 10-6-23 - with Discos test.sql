SELECT ORDNO, ENV, PON, ACT, ORD_TYPE, SERV_TYPE, STATUS, WTN, BTN, SE_CODE, NETWORK, CCNA, 
       ...
       ADDRESS, CITY, STATE, SUM(MRC) TOTAL_MRC, MSOSTS, SV7STS
   FROM (
SELECT ORDNO, ENV, PON, ACT, ORD_TYPE, SERV_TYPE, STATUS, WTN, BTN, SE_CODE, LC.COP_FIB NETWORK, SQ.CCNA, 
       CL.CARRIER_NAME CARRIER, CUSTNM, SO_CREATE_DT, SO_EFF_COMP_DT, SO_ACT_COMP_DT, POST_DT, AGING,
       ...
       SV7.SV7RTE AS MRC, SV7.SV7ITM AS SE_CODE_MRC, SQ.MSOSTS, SQ.SV7STS 
   FROM (         
SELECT DISTINCT A.BILL_SUB_SYS ENV, MSOSO# AS ORDNO, E.XSOPON PON, F.PONACT ACT, 
       ...
       MSOPH# AS WTN, MSOMB# AS BTN, SV7ITM AS SE_CODE, E.XSODEC CCNA, G.CARRIER_NAME, MSOLSN||MSOFRN AS CUSTNM,  
       CAST(TO_CHAR(MSOSDT) AS DATE FORMAT 'YYYYMMDD') SO_CREATE_DT,
       CASE WHEN MSOEDT > 0 THEN CAST(TO_CHAR(MSOSDT) AS DATE FORMAT 'YYYYMMDD') ELSE NULL END SO_EFF_COMP_DT, 
       ...
       TRIM(MSOST#)||' '||TRIM(MSOSTR)||' '||TRIM(MSOSST) AS ADDRESS, CTYDES AS CITY, MSOSTA AS STATE, MSOSTS, SV7STS
...
LEFT OUTER JOIN USER_WORK.CM_CARRIER_LIST G
 ...
 GROUP BY ORDNO, ENV, PON, ACT, ORD_TYPE, SERV_TYPE, STATUS, WTN, BTN, SE_CODE, NETWORK, CCNA, 
          CARRIER, CUSTNM, SO_CREATE_DT, SO_EFF_COMP_DT, SO_ACT_COMP_DT, POST_DT, AGING, AGING_GRP,
          ADDRESS, CITY, STATE, MSOSTS, SV7STS
