SELECT DISTINCT BILL_MONTH_DATE, CARRIER, CCNA, STATE, TARIFF_CD, BAN_TYPE, WTN, BTN, STN, 
       GL_MATRIX_CLASS, PRD_SVC_CD, PRD_SVC_CD_DS, TOTAL_MRC, CUSTNAME
FROM (
-- 8
SELECT DISTINCT CARRIER, CCNA, STATE, TARIFF_CD, BAN_TYPE, BILL_MONTH_DATE, WTN, BTN, STN, 
       GL_MATRIX_CLASS, PRD_SVC_CD, PRD_SVC_CD_DS, VOICE_MRC USOC_MRC, TOT_CHRGE_AMT TOTAL_MRC, CUSTNAME,
...
SELECT 
   (SELECT SUM((CHRGE_AMT)-(PRORATED_CHARGE_AMT)) FROM EDW_VWMC.EUBS_BILL_REV_ACCT_DETAIL_V AS EUBS 
        ...
        AND GL_MATRIX_NO IN (SELECT GL_MATRIX_NO FROM USER_WORK.CM_GL_MATRIX WHERE GL_MATRIX_CLASS = 'DATA SERVICES')) AS TOT_CHRGE_AMT,
        SQ6.*
       FROM ( 
-- 6
SELECT DISTINCT CARRIER, CCNA, 
       STATE, TARIFF_CD, BAN_TYPE, BILL_MONTH_DATE, SQ5.WTN, SQ5.BTN, MASTERACCOUNTNUMBER STN, 
...
-- 5 
SELECT DISTINCT CARRIER, CCNA, 
       STATE, TARIFF_CD, BAN_TYPE, WTN, BTN, MASTERACCOUNTNUMBER, CUSTNAME,  BILL_MONTH_DATE,
...
SELECT CASE WHEN CARRIER_NAME IS NOT NULL THEN CARRIER_NAME ELSE CARRIER END CARRIER, 
       CCNA, STATE, TARIFF_CD, BAN_TYPE, WTN, BTN, MASTERACCOUNTNUMBER, CUSTNAME, 
...
SELECT MCL.PRIMARY_CARRIER_NAME CARRIER, CARLIS.CARRIER_NAME, SQ2.CCNA, BAN_STATE STATE, TARIFF_CD, CUSTNAME, 
       BAN_TYPE, WTN, BTN, MASTERACCOUNTNUMBER
FROM (
-- 2
SELECT LSPBAN.BANCARCD CCNA, SQ1.WTN, SQ1.BTN, SQ1.MASTERACCOUNTNUMBER, CUSTNAME, 
       SQ1.STATE, LSPBAN.BANSTACD BAN_STATE, LSPBAN.BANTRFCD TARIFF_CD,
...
WHERE MASTERACCOUNTNUMBER IS NOT NULL
    -- AND C2F.TELEPHONENUMBER in ('3103189242')
-- 1
)SQ1,
...
-- 5 
 ...
 WHERE GL_MATRIX_CLASS = 'DATA SERVICES'
  ...
  AND WTN <> STN
  -- AND SUBHIST.SEQ = '0'
 GROUP BY CARRIER, CCNA, STATE, TARIFF_CD, BAN_TYPE, BILL_MONTH_DATE,
