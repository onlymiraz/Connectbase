select distinct ckt, state, svc_cd,  
	   ...
	   case when substr(svc_cd,1,2) in ('HC','HX','T1') then 'DS1'
            ...
            when substr(svc_cd,1,2) in ('SX','KG','KS') then 'Ethernet-NNI'
       ...
       case when svc_cd in ('KD','KP') then '1G'  --10M, but counted as 1G 
            when svc_cd in ('KE','KQ') then '1G'  --100M, but counted as 1G     			
...
select ckt, icsc, rate, type,
       case when ccna1 is not null then ccna1
       ...
       case when state1 is not null and state1 <> 'EF' then state1
            ...
            when icsc = 'FV03' and substr(ckt,1,2) = '83' then 'ID'
            when icsc = 'FV03' and substr(ckt,1,2) in ('43','85') then 'OR'
            when icsc = 'FV03' and substr(ckt,1,2) = '86' then 'WA'
            else state3 end state,     
	   ocn, svc_cd, status, juris, ordnum, lastmoddt		
from (
select exchange_carrier_circuit_id ckt,
       ...
       max(d.last_modified_date)  keep (dense_rank last order by d.last_modified_date) lastmoddt
...
select exchange_carrier_circuit_id ckt,
     ...
     max(c.rate_code) keep (dense_rank last order by c.last_modified_date) rate, 
	   ...
	   max(substr(nl1.clli_code,5,2)) keep (dense_rank last order by nl1.last_modified_date) state2,
	   ...
	   max(d.last_modified_userid)  keep (dense_rank last order by d.last_modified_date) lastmod,
       max(d.last_modified_date)  keep (dense_rank last order by d.last_modified_date) lastmoddt
from circuit c, design d, network_location nl1, ASAP.design_ord_summ dos ,
     asap.serv_req sr, asap.access_service_request asr, npa_nxx npa,
     (SELECT DISTINCT *
...
group by c.exchange_carrier_circuit_id, ga.instance_value_abbrev
))
where substr(ckt,7,1) <> 'U'
and status not in ('8','A')
and state in ('OR','WA','ID','MT')
order by 1;
