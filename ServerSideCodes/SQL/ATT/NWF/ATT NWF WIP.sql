select DISTINCT docno, pon, req, act, acna, comp, ckt, clean, dd, accept_dt, dd_comp,
       state, sei, evc_ind, prod, bdw, product, task_at_jeop, jeop_code
from (      
select document_number docno, pon, req, act, acna, comp, clean, dd, accept_dt, dd_comp, state,
       min(ckt) ckt, 
...
from (
select a.document_number, trunc(clean) clean, dd, accept_dt, dd_comp,  
...
where a.document_number = tjw.document_number (+)
  ...
  and une <> 'Y'
group by a.document_number, clean, dd, accept_dt, dd_comp, nc, svc_cd, ckt, evc_ind,  
        pon, acna, GA_STATE, STATE, sei, act, UNE, evc_ind, req, svc_cd, nc, ckt  
)
group by document_number, pon, req, act, acna, comp, clean, dd, accept_dt, dd_comp, state, sei, evc_ind, 
         prod, bdw, task_at_jeop, JEOP_CODE
)
