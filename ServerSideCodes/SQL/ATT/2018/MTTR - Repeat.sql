...
select DISTINCT TICKET_ID, STATE, REGION, CLEC_ID, COMPANY, CKT_ID, PRODUCT, PROD2, BDW, CREATE_DATE,
       ...
       CASE WHEN DISP IN ('CO','FAC') THEN 'Y' ELSE 'N' END FOUND, cause_cd, to_char(closed_dt,'MM') mon
from (
...
where a.fld_troublereportstate = 'closed'
 ...
 and (to_char(dte_closeddatetime,'yyyymm') in ('202011','202012')    --NEED TO CHANGE THIS EACH MONTH
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202011','202012')))   --NEED TO CHANGE THIS EACH MONTH
 and e.status (+) = '6'
...
select region, state, company, product, t2.ckt_id, t2.ticket_id, disp, found, create_date, cleared_dt, closed_dt, days, rpt  
