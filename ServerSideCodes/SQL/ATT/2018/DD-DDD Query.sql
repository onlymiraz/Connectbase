...
select docno, req, act, build, build_iof, build_osp,  
       ...
       acna, comp, WHY_MISS, miss_reason, proj, state, region, product, 
...
where sr.document_number = asr.document_number
  ...
  AND ASR.NPA = NPA.NPA (+)
  and asr.nxx = npa.nxx (+)
  and to_char(t.actual_completion_date,'yyyymm') = '202012'    --Current Reporting Month 
...
select a.docno, req, act, build, build_iof, build_osp,  
       ...
       case when product in ('ATX DS1','ATX DS3','ATX OCN') then 'ATX TDM'
            when product in ('ATX Ethernet UNI','ATX Ethernet Pop to Prem','ATX Ethernet Combo') then 'ATX UNI ALL'
            when product = 'ATX Ethernet VLAN' then 'ATX EVC'
            ...
            when comp = 'MOB' and prod = 'Ethernet' and act = 'D' then 'MOB Ethernet Disconnects'
...
and product <> 'EXCLUDE')
and comp||act not in ('ATXD')
and comp||act||prod not in ('MOBDDS1','MOBDDS3','MOBDOCN');
*/     
