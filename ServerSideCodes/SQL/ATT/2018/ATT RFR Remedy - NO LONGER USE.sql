...
select DISTINCT TICKET_ID, STATE, REGION, CLEC_ID, 
       case when clec_id in ('ATX','AAV','SBB','SBZ','SUV','TPM','LOA','AVA','AYA') then 'ATX'||product else 'MOB'||product end company,
...
select ticket_id, state, clec_id, cl.customer, ckt_id, product, create_date, cleared_dt, closed_dt,  
	   ...
	   case WHEN STATE IN ('CT','NY','PA') then 'East'
	        WHEN STATE IN ('IA','IL','IN','MI','MN','NE','OH','WV','WI','KY','MD','VA') then 'Midwest'
...
where a.fld_troublereportstate = 'closed'
 ...
 and (to_char(dte_closeddatetime-5/24,'yyyymm') in ('201801','201802')    --NEED TO CHANGE THIS EACH MONTH
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('201801','201802')))   --NEED TO CHANGE THIS EACH MONTH
