  ...
  from (
    select distinct prvsn_uid,  
	  case when substr(prchs_order_no,3,1) = 'H' then 'Inter-office Facilities'
	   when (substr(nc,1,1) in ('K','V') and prv.acna = 'ATX') then 'ATX Ethernet'
	   ...
	   when (substr(nc,1,2) = 'HC' 
	    ...
	    when (substr(nc,1,1) in ('K','V')
		   and prv.acna in ('AAX','ACF','ACH','ADM','AEC','AGS','AGZ','AHA','AHD','AHM','AHO',
			 ...
			 'SBG','SBM','SBN','SCU','SHI','SLL','SMC','SNP','STH','SUF','SWM',
			 'SWP','SWT','SWV','SYC','SZM','TGH','TQU','UMT','VGD','VRA',
			 'WBT','WGL','WLG','WLZ','WVO','WWC')) then 'MOB Ethernet'	 			 	 
...
and prv.to_cd <> ' '
and substr(prv.clli_z,5,2) in ('AZ','CA','ID','IL','IN','MI','NC','NV','OH','OR','SC','WA','WI')
