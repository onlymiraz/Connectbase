...
drop table ncfdetail   

CREATE TABLE ncfdetail NOLOGGING NOCACHE AS    
	select distinct prvsn_uid, min(repair_uid) repair_uid
    from OSSAMS_MART.TB_DM_OSSAMS_REPAIR_FACT rep, OSSAMS_MART.TB_DM_OSSAMS_PRVSN_FACT prv
where prv.ecckt = rep.ckt_id(+)
and TO_CHAR(prv.LOAD_DATE, 'YYYYMM') = '201011'  --CHANGE DATES (MONTH IN ARREARS)
and substr(clli_z,5,2) in ('AZ','CA','ID','IL','IN','MI','NC','NV','OH','OR','SC','WA','WI')  
and prv.z_action in ('A','C','I','N','T')
and prv.invalid_ind not in ('I')
and prv.to_cd <> ' '
and prv.ccna in ('ATX','TPM','ACC','ANF','ETL','IPS','AAX','ACF','ACH','ADM','AEC',
             'AGS','AGZ','AHA','AHD','AHM','AHO',
...
and TO_CHAR(rep.REPORT_DATE,'YYYYMMDD') between '20101101' and '20101231'  --CHANGE DATES  Current Month (in arrears) through Next Month   
...
and rep.state in ('AZ','CA','ID','IL','IN','MI','NC','NV','OH','OR','SC','WA','WI')
and rep.ACNA in ('ATX','TPM','ACC','ANF','ETL','IPS','AAX','ACF','ACH','ADM','AEC',
             'AGS','AGZ','AHA','AHD','AHM','AHO',
	   		 'AIL','AIN','AIS','AKZ','ALY','AMH','AMP','AWL','AWN','AZE','BAC',
			 'BAK','BAO','BCU','BFL','BGH','BPN','BSM','CBL','CCB','CDA','CEO',
			 'CEU','CFN','CIV','CIW','CKQ','CLQ','COW','CQW','CRF','CRJ','CSG',
...
select 'E.2.5', 'NCF', '01-Dec-2010', '31-Dec-2010',
    case when substr(prchs_order_no,3,1) = 'H' then 'Inter-office Facilities'
	   when (substr(nc,1,1) in ('K','V') and prv.acna = 'ATX') then 'Wireline Ethernet'
	   ...
	   when (substr(nc,1,2) = 'HC' and substr(prchs_order_no,4,1) in ('P','Y') and prv.acna in ('TPM','ACC','ANF','ETL','IPS')) then 'TPM DS1'
	   ...
	   when (substr(nc,1,2) = 'HF' and substr(prchs_order_no,4,1) in ('P','Y') and prv.acna in ('TPM','ACC','ANF','ETL','IPS')) then 'TPM DS3'
	   ...
	   when (substr(nc,1,2) = 'HF' 
	       and prv.acna in ('AAX','ACF','ACH','ADM','AEC','AGS','AGZ','AHA','AHD','AHM','AHO',
			 ...
			 'SBG','SBM','SBN','SCU','SHI','SLL','SMC','SNP','STH','SUF','SWM',
			 'SWP','SWT','SWV','SYC','SZM','TGH','TQU','UMT','VGD','VRA',
			 'WBT','WGL','WLG','WLZ','WVO','WWC')) then 'MOB DS3'
	   when (substr(nc,1,2) = 'HC' 
	       and prv.acna in ('AAX','ACF','ACH','ADM','AEC','AGS','AGZ','AHA','AHD','AHM','AHO',
			 ...
			 'SBG','SBM','SBN','SCU','SHI','SLL','SMC','SNP','STH','SUF','SWM',
			 'SWP','SWT','SWV','SYC','SZM','TGH','TQU','UMT','VGD','VRA',
			 'WBT','WGL','WLG','WLZ','WVO','WWC')) then 'MOB DS1'
		when (substr(nc,1,1) in ('K','V') 
		  and prv.acna in ('AAX','ACF','ACH','ADM','AEC','AGS','AGZ','AHA','AHD','AHM','AHO',
			 ...
			 'SBG','SBM','SBN','SCU','SHI','SLL','SMC','SNP','STH','SUF','SWM',
			 'SWP','SWT','SWV','SYC','SZM','TGH','TQU','UMT','VGD','VRA',
			 'WBT','WGL','WLG','WLZ','WVO','WWC')) then 'Wireless Ethernet'	 			 	 
	    else 'UNDEFINED' end service, 
	   'FTR13', prchs_order_no, ecckt, ckr, order_no, due_sched_date, dsrd_date, due_cmpln_date, why_miss, NULL,
       prjct_number, rca_detail analysis, NULL, to_char(report_date,'MM/DD/YYYY') trbl_date  
from OSSAMS_MART.TB_DM_OSSAMS_REPAIR_FACT rep, OSSAMS_MART.TB_DM_OSSAMS_PRVSN_FACT prv, OSSAMS_MART.TB_OSSAMS_RCA_DETAIL rca
where prv.ecckt = rep.ckt_id(+)
 and prv.prvsn_uid = rca.record_uid(+)
and prvsn_uid in(
  select prvsn_uid  
    from ncfdetail)
and repair_uid in (
