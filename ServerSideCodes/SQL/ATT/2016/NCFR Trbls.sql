...
select distinct ticket_id, 
	   ...
	   case when to_char(Create_Date,'yyyymmdd') between '20170312' AND '20171104' then Create_Date-4/24 
	        else Create_Date-5/24 end create_date, 
	   case when to_char(Cleared_Dt,'yyyymmdd') between '20170312' AND '20171104' then Cleared_dt-4/24
            when Cleared_Dt is not null then Cleared_dt-5/24
            when cleared_dt is null and to_char(Closed_Dt,'yyyymmdd') between '20170312' AND '20171104' then Closed_dt-4/24
	        else Closed_dt-5/24 end Cleared_Dt, 
	   case when to_char(Closed_Dt,'yyyymmdd') between '20170312' AND '20171104' then Closed_dt-4/24
            when Closed_Dt is not null then Closed_dt-5/24
            when closed_dt is null and to_char(Cleared_Dt,'yyyymmdd') between '20170312' AND '20171104' then Cleared_dt-4/24
	        else Cleared_dt-5/24 end Closed_Dt,
       Total_Duration, 
	   TTR,
