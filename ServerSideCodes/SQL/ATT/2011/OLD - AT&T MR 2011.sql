...
where TO_CHAR(AGE_DATE, 'YYYYMMDD') in ('20110801')   --CHANGE DATE   
...
or to_char(a.load_date, 'YYYYMMDD') = '20110801'
...
group by ID, SC
UNION ALL
select 'RFR', ID||' '||sc svc, round((sum(mttr)/60),2) num, count(*) denom 
from attmr
--where disp = 'NF'
group by ID, SC
UNION ALL
