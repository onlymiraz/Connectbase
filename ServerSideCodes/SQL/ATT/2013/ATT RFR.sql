...
SELECT DISTINCT REGION, STATE, DOCNO, FTR_TKT, PRODUCT, CREATE_DATE, CLEARED_DT, CLOSE_DT, TT_TYPE, CURRENT_STATE,
	   ...
	   SERV_CODE, TROUBLE_FOUND_CD, TRBL_FOUND_ID, DISPOSITION, FOUND, TTR
FROM (
SELECT 
...
SELECT MAX(C.RATE_CODE) KEEP (DENSE_RANK LAST ORDER BY C.LAST_MODIFIED_DATE) AS PRODUCT_LEVEL,
 ...
 AND TO_CHAR(ASAP.PKG_GMT.SF_GMT_AS_PASSED_TIMEZONE(TT.CLOSE_DT,'1222'),'yyyymm') in ('201405','201406')
...
-- WHERE (SUBSTR(ICSC,1,2) = 'FV' OR ICSC IS NULL)
-- AND STATE IN ('WV','MD','AZ','CA','ID','IL','MO','IN','KY','MI','NC','NV','OH','OR','SC','WA','WI') 
   ORDER BY REGION, STATE, TTR DESC
   

 
select region, state, company, circuit, found, create_date, ttr, to_char(close_dt,'mm') mon
from attrfr
order by 4,6 
