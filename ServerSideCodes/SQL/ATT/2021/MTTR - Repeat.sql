...
select distinct ticket_id, state, clec_id, ckt_id, product, 
       ...
       case when product = 'Ethernet' and service_type_code in ('KF','KR') then '1G'
            ...
            when product = 'Ethernet' and service_type_code in ('KJ') then '10G'       --100G, BUT COUNTED AS 10G 
...
where a.fld_troublereportstate = 'closed'
 ...
 and (to_char(dte_closeddatetime,'yyyymm') in ('202211','202212')    --NEED TO CHANGE THIS EACH MONTH
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202211','202212')))   --NEED TO CHANGE THIS EACH MONTH
 and e.status (+) = '6'
...
--MTTR    
select * from attmr
where mon = '12'   --NEED TO CHANGE THIS EACH MONTH;
...
select company, product, bdw, region, count(*) cnt
from (
select ticket_id, state, region, clec_id, company, product, bdw,
...
from (
select r.ticket_id, r.state, r.region, r.clec_id, r.company, 
...
group by r.ticket_id, r.state, r.region, r.clec_id, r.company, 
         r.product, r.ckt_id, r.create_date, r.cleared_dt, r.closed_dt, r.disp, r.bdw
)
where create_date-prev_clear_dt <=30
)
group by company, product, bdw, region
order by 1,2,3,4;
...
select ticket_id, state, clec_id, company, product,
       ckt_id, create_date, cleared_dt, closed_dt, disp, prev_ticket_id, prev_create_dt, prev_clear_dt,
       prev_disp, round(create_date-prev_clear_dt,0) days, to_char(closed_dt,'MM') mon 
from (
