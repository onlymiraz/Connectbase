...
PROJECT_ID, NC, NC_OPT, REQ, SEI, SPEC, LREF_SPEC, ICSC, RUID1, RUID2, LOS, EXPEDITE, DESIGN_ID,
BUILD_OSP1, BUILD_IOF1, BUILD_GRP, ETH_PROD, CABS_TIER, PRODUCT, ORDER_RECD_MONTH, ORDER_COMP_MONTH, INIT_ASR,
FIRST_CLEAN_ASR, CLEAN_ASR, ORIG_CONF_DT, ASRCONF_DT, COMPLETION_DT, DD_TASK_COMP, EXCLUDE, WMRCA, WMRCA_DESC, DD_STATUS, ATT_PRODUCT,
CASE WHEN ATT_PRODUCT IN ('ATX DS1','ATX DS3','ATX OCN') AND ECCKT_TYPE <> 'CLF' AND ACT_IND = 'N' THEN 'ATX TDM'
     -- WHEN ATT_PRODUCT IN ('MOB DS1','MOB DS3','MOB OCN') THEN 'MOB TDM'
     WHEN ATT_PRODUCT IN ('ATX Ethernet UNI','ATX Ethernet Pop to Prem','ATX Ethernet Combo') AND ACT_IND = 'N' THEN 'ATX UNI ALL'
     WHEN SUBSTR(ATT_PRODUCT,1,12) = 'MOB Ethernet' THEN 'MOB ETHERNET'
     ELSE NULL END ATT_PRODUCT2,
CASE WHEN COMPLETION_DT-INIT_ASR > 0 THEN COMPLETION_DT-INIT_ASR ELSE 1 END "CAL - INIT ASR TO COMPLETION",
CASE WHEN COMPLETION_DT-FIRST_CLEAN_ASR > 0 THEN COMPLETION_DT-FIRST_CLEAN_ASR ELSE 1 END "CAL - FIRST CLEAN ASR TO COMPLETION",
CASE WHEN COMPLETION_DT-CLEAN_ASR > 0 THEN COMPLETION_DT-CLEAN_ASR ELSE 1 END "CAL - FINAL CLEAN ASR TO COMPLETION",
CASE WHEN ORIG_CONF_DT-INIT_ASR > 0 THEN ORIG_CONF_DT-INIT_ASR ELSE 1 END "CAL - INIT ASR TO 1ST FOC",
CASE WHEN ORIG_CONF_DT-FIRST_CLEAN_ASR > 0 THEN ORIG_CONF_DT-FIRST_CLEAN_ASR ELSE 1 END "CAL - FIRST CLEAN ASR TO 1ST FOC" 
...
STATE, EU_NAME, EU_ADDRESS, EU_CITY, EU_STATE, EU_ZIPCD, EO_CLLI, LATA, P_NUM, PROJECT_ID, NC, NC_OPT, REQ, SEI, SPEC, LREF_SPEC, ICSC,
...
CLEAN_ASR, ORIG_CONF_DT, ASRCONF_DT, COMPLETION_DT, DD_TASK_COMP, EXCLUDE, WMRCA, 
CASE WHEN WMRCA = 'PM54' THEN '48 Hr Customer Test' ELSE WMRCA_DESC END WMRCA_DESC,
...
CASE WHEN ACNA = 'ATX' AND CIRCUIT_TYPE = 'DS1' AND substr(pon,3,1) = 'S' and substr(pon,4,1) = '0' then 'ATX DS1'
     WHEN ACNA = 'ATX' AND CIRCUIT_TYPE = 'DS1' AND substr(pon,3,1) = 'S' and substr(pon,4,1) = 'X' then 'ATX DS1 ESO'
     when acna = 'TPM' and CIRCUIT_TYPE = 'DS1' and substr(pon,4,1) in ('P','Y') then 'ATX DS1'
     when acna in ('SBB','SBZ','AAV','SUV') and CIRCUIT_TYPE = 'DS1' then 'ATX DS1'
     when ACNA = 'ATX' and CIRCUIT_TYPE = 'DS3' and substr(pon,3,1) = 'S' and substr(pon,4,1) = '0' then 'ATX DS3'
     when ACNA = 'ATX' and CIRCUIT_TYPE = 'DS3' and substr(pon,3,1) = 'S' and substr(pon,4,1) = 'X' then 'ATX DS3 ESO'
     when acna = 'TPM' and CIRCUIT_TYPE = 'DS3' and substr(pon,4,1) in ('P','Y') then 'ATX DS3'
	 when acna in ('SBB','SBZ','AAV','SUV') and CIRCUIT_TYPE = 'DS3' then 'ATX DS3'
     when acna = 'ATX' and CIRCUIT_TYPE = 'OCN' and substr(pon,3,1) = 'S' and substr(pon,4,1) = '0' then 'ATX OCN'
     when acna = 'ATX' and EVCI = 'B' then 'ATX Ethernet Combo'
     when acna = 'ATX' and substr(nc_ASR,1,1) = 'K' and req = 'E' and SEI = 'Y' then 'ATX Ethernet UNI'
     when acna = 'ATX' and substr(nc_ASR,1,1) = 'K' and req = 'S' and sei = 'Y' then 'ATX Ethernet Pop to Switch'
     when acna = 'ATX' and substr(nc_ASR,1,1) = 'K' and req = 'S' and sei is null then 'ATX Ethernet Pop to Prem'
     when acna = 'ATX' and EVCI = 'A' then 'ATX Ethernet VLAN'
     when acna = 'ATX' and substr(pon,4,1) = 'V' then 'ATX Ethernet VLAN'    
	 WHen acna = 'ATX' and substr(pon,4,1) = 'C' then 'ATX Ethernet Combo'
     when PRIMARY_CARRIER_NAME = 'AT'||chr(38)||'T MOBILITY' and substr(CIRCUIT_ID,4,1) = 'V' then 'MOB Ethernet EVC'
	 when PRIMARY_CARRIER_NAME = 'AT'||chr(38)||'T MOBILITY' and substr(CIRCUIT_ID,4,1) = 'K' and req = 'S' then 'MOB Ethernet MTSO'
	 when PRIMARY_CARRIER_NAME = 'AT'||chr(38)||'T MOBILITY' and substr(CIRCUIT_ID,4,1) = 'K' and req = 'E' then 'MOB Ethernet UNI' 
     ELSE NULL END ATT_PRODUCT,
...
WMRCA, RCA_JEOP_TYPE, WMRCA_DESC, EVCI, REQ,NC_ASR,
...
ASR.NETWORK_CHANNEL_SERVICE_CODE AS nc_asr,
...
AND (ASR.SUPPLEMENT_TYPE <> '1' OR ASR.SUPPLEMENT_TYPE IS NULL)) ASR
...
--AND SR.DOCUMENT_NUMBER IN ('3814960','3814976')
--and sr.document_number in ('3683144','3699255','3664911')
AND SR.ACTIVITY_IND IN ('N','C','M')
