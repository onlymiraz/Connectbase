...
create table trouble nologging nocache as
...
where a.exchange_carrier_circuit_id = e.EXCHANGE_CARRIER_CIRCUIT_ID(+) 
 ...
 and e.status (+) = '6'
 and (to_char(dte_closeddatetime,'yyyymm') in ('202306','202307')    --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS)
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202306','202307')))   --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS) 
...
select ticket_id, state, region, territory, director, clec_id, customer, ckt_id, product, create_date, cleared_dt, closed_dt, 
       ttr, repair_code, disp, site_clli, profile, to_char(closed_dt,'MM') mon
  from trouble
  where mon = '07'     --NEED TO CHANGE THIS EACH MONTH
...
create table repeat nologging nocache as
select * 
from trouble
where mon = '07'   --NEED TO CHANGE THIS EACH MONTH
and disp in ('CO','FAC');
...
select ticket_id, state, region, territory, director, site_clli, clec_id, customer, product,
...
--FOR REPEAT DENOM, RUN THIS QUERY AND PASTE RESULTS   

select region, territory, product, count(*) den, mon
from trouble where mon = '07'          --NEED TO CHANGE THIS EACH MONTH
group by region, territory, product, mon
union all
select territory, territory, product, count(*) den, mon
from trouble where mon = '07'          --NEED TO CHANGE THIS EACH MONTH
group by territory, territory, product, mon
union all
select director, territory, product, count(*) den, mon
from trouble where mon = '07'           --NEED TO CHANGE THIS EACH MONTH
group by director, territory, product, mon
order by 1,3;


--FOR THE DAILY MR SCRUB FILE 

select ticket_id, state, territory, region, clec_id, customer, ckt_id, product, create_date, cleared_dt, closed_dt, 
       ttr, repair_code, disp, clli_code site_clli, profile, location_name
  from trouble
  where mon = '07'     --NEED TO CHANGE THIS EACH MONTH
  and (disp in ('CO','FAC','CC','NTF') or disp is null)
  order by 4,2,1;  
