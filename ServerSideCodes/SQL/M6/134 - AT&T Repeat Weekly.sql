...
drop table ATTRFRFWEEKLY
/

CREATE TABLE ATTRFRFWEEKLY NOLOGGING NOCACHE AS
select DISTINCT TICKET_ID, STATE, REGION, CLEC_ID, COMPANY, CKT_ID, PRODUCT, PROD2, BDW, CREATE_DATE,
       ...
       CASE WHEN DISP IN ('CO','FAC') THEN 'Y' ELSE 'N' END FOUND, 
       cause_cd, to_char(closed_dt,'MM') mon
from (
select ticket_id, state, clec_id, cl.customer, ckt_id, product,
       case when product in ('DS1','DS3','OCN') and company = 'ABS' then 'ABS TDM'
            when product in ('Ethernet') and company = 'ABS' then 'ABS Ethernet'       
            when product in ('DS1','DS3','OCN','EVC') and company = 'MOB' then 'Exclude'  
	   ...
	   case WHEN STATE IN ('NY','PA','CT','OH','WV','VA','MD') THEN 'East'
...
select distinct ticket_id, state, clec_id, ckt_id, product, 
       case when clec_id in ('ATX','AAV','SBB','SBZ','SUV','TPM','LOA','AVA','AYA') then 'ABS' else 'MOB' end company,
...
where a.fld_troublereportstate = 'closed'
 ...
 and (e.ecckt_type <> 'CLF' or e.ecckt_type is null)
 and (to_char(dte_closeddatetime,'yyyymm') in ('202312','202401')    --NEED TO CHANGE THIS EACH MONTH - use both previous and current months 
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202312','202401')))   --NEED TO CHANGE THIS EACH MONTH - use both previous and current months 
 and e.status (+) = '6'
 and a.fld_requestid not in ('OP-000003521516')
...
and prod2 in ('ABS TDM','ABS Ethernet','MOB 1G','MOB 10G')
order by 4,3,1;
...
select prod2, count(*) 
from attrfrfweekly
where mon = '01'   --NEED TO CHANGE THIS EACH MONTH
...
drop table attrepeatweekly
/

create table attrepeatweekly nologging nocache as
select * 
from attrfrfweekly
where mon = '01'   --NEED TO CHANGE THIS EACH MONTH
...
select ticket_id, state, clec_id, company, product, prod2,
       ...
       case when DISP in ('NTF','CC') then 'Yes' else null end "NTF Repeat"  
from (
select r.ticket_id, r.state, r.clec_id, r.company,
       r.product, r.prod2, r.ckt_id, r.create_date, r.cleared_dt, r.closed_dt, r.disp, 
...
from attrepeatweekly r, attrfrfweekly t 
