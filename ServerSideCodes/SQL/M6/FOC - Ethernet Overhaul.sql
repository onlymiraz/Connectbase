select to_char(response_date,'MM') mon, isccode, ccna, 
       ...
       case when state in ('ID','MT') then 'ID/MT'
            when state in ('AL','GA','MS','TN') then 'South States'
...
select isccode, ccna, pon, status, request_date, response_date, reqtype, act, document_number,
       ...
       NC, acna, ckt, spec, evc_ind, BDT_CLOSE, UNE, BB_REQD
   from (     
select isccode, ccna, pon, status, request_date, response_date, reqtype, act, state, acna, document_number, UNE,
       ...
       evc_ind, SPEC, substr(exchange_area_clli,5,2) npastate, BDT_CLOSE, BB_REQD 
   FROM (     
select isccode, a.ccna, a.pon, a.status, request_date, response_date, reqtype, activity, state,
       ...
       MAX(AUD.BROADBAND_REQUIRED) KEEP (DENSE_RANK LAST ORDER BY AUD.LAST_MODIFIED_DATE) BB_REQD,
...
from (
select isccode, ccna, pon, 'Confirmed' status,  reqtype, activity, requeststate state,
min(submitteddatetime) request_date, 
min(updatedatetime) response_date
from whsl_adv_hist.vfo_orderhistoryinfo_thist vfo
where orderstatus in ('Confirmed_Submitted','Confirmed_Sent')
and to_char(updatedatetime,'yyyymm') = '201812'
and to_char(submitteddatetime,'yyyy') >= '2015'
and isccode <> 'FTRORD'
and ccna not in ('FLR','ZZZ','ZTK','CUS')
and substr(reqtype,1,1) in ('E','S')
group by isccode, ccna, pon, reqtype, activity, requeststate
) a, 
...
and prod in ('Ethernet-UNI','Ethernet-NNI','Ethernet-Combo','Ethernet-EVC')
