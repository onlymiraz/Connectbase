...
CASE WHEN LATA IN ('552','730','740','952','961','973') THEN 'Carrier CTF' 
     WHEN LATA IS NOT NULL THEN 'Carrier FTTC'
     WHEN ICSC IN ('GT10','GT11') THEN 'Carrier CTF' ELSE 'Carrier FTTC' END PRODUCT,
HBE_CIRCUIT, EVC_DESIGN_ID, EVC_CKT, UNI_CKT, NNI_CKT, 
...
REPLACE(ADDRESS,',','') ADDRESS, REPLACE(CITY,',','') CITY,  REPLACE(STATE,',','') STATE, 
LATA, ICSC, PNUM,  DOCUMENT_NUMBER, DD_COMP, ACT, EVC_BDW, LOS, EVC_SPEC, EVC_LEVEL, EVC_CKR, HBE_CIRCUIT,
...
-- 6 
SELECT BAN, ACNA, CUSTOMER,
REPLACE(ADDRESS,'MSAG ') AS ADDRESS, CITY, STATE, LATA_NUMBER LATA, ICSC,
PNUM, DD_COMP, DOCUMENT_NUMBER, ACT, EVC_BDW, LOS, EVC_SPEC, EVC_LEVEL, EVC_CKR, HBE_CIRCUIT,
...
CASE WHEN UNI_NID = 'YES' THEN UNI_LOC_ID 
    WHEN UNI_NID2 = 'YES' THEN UNI_LOC_ID2 
    WHEN UNI_H_COORD IS NOT NULL THEN UNI_LOC_ID ELSE UNI_LOC_ID2 END UNI_LOC_ID,
CASE WHEN UNI_NID = 'YES' THEN UNI_CLLI 
    WHEN UNI_NID2 = 'YES' THEN UNI_CLLI2 
    WHEN UNI_H_COORD IS NOT NULL THEN UNI_CLLI ELSE UNI_CLLI2 END UNI_CLLI,
CASE WHEN UNI_NID = 'YES' THEN UNI_LOC 
    WHEN UNI_NID2 = 'YES' THEN UNI_LOC2 
    WHEN UNI_H_COORD IS NOT NULL THEN UNI_LOC ELSE UNI_LOC2 END UNI_LOC,
CASE WHEN UNI_NID = 'YES' THEN UNI_H_COORD 
    WHEN UNI_NID2 = 'YES' THEN UNI_H_COORD2 
    WHEN UNI_H_COORD IS NOT NULL THEN UNI_H_COORD ELSE UNI_H_COORD2 END UNI_H_COORD,
CASE WHEN UNI_NID = 'YES' THEN UNI_V_COORD 
    WHEN UNI_NID2 = 'YES' THEN UNI_V_COORD2 
    WHEN UNI_H_COORD IS NOT NULL THEN UNI_V_COORD ELSE UNI_V_COORD2 END UNI_V_COORD,
CASE WHEN NNI_NID = 'YES' THEN NNI_LOC_ID 
    WHEN NNI_NID2 = 'YES' THEN NNI_LOC_ID2 
    WHEN NNI_H_COORD IS NOT NULL THEN NNI_LOC_ID ELSE NNI_LOC_ID2 END NNI_LOC_ID,
CASE WHEN NNI_NID = 'YES' THEN NNI_CLLI 
    WHEN NNI_NID2 = 'YES' THEN NNI_CLLI2 
    WHEN NNI_H_COORD IS NOT NULL THEN NNI_CLLI ELSE NNI_CLLI2 END NNI_CLLI,
CASE WHEN NNI_NID = 'YES' THEN NNI_LOC 
    WHEN NNI_NID2 = 'YES' THEN NNI_LOC2 
    WHEN NNI_H_COORD IS NOT NULL THEN NNI_LOC ELSE NNI_LOC2 END NNI_LOC,
CASE WHEN NNI_NID = 'YES' THEN NNI_H_COORD 
    WHEN NNI_NID2 = 'YES' THEN NNI_H_COORD2 
    WHEN NNI_H_COORD IS NOT NULL THEN NNI_H_COORD ELSE NNI_H_COORD2 END NNI_H_COORD,
CASE WHEN NNI_NID = 'YES' THEN NNI_V_COORD 
    WHEN NNI_NID2 = 'YES' THEN NNI_V_COORD2 
    WHEN NNI_H_COORD IS NOT NULL THEN NNI_V_COORD ELSE NNI_V_COORD2 END NNI_V_COORD
FROM (
-- 5 
SELECT EVC_CKT, EVC_DESIGN_ID, ACNA, BAN, A.DOCUMENT_NUMBER, EVC_IND, ACT, DD_COMP, UNI_SPEC, PNUM, LOS, EVC_SPEC, HBE_CIRCUIT,
       LATA, ADDRESS, CITY, STATE, EVC_BDW, EVC_LEVEL, EVC_STATUS, EVC_STATUS_DESC, EVC_CKR, ICSC,
       UNI_CKT, C2A.STATUS UNI_STATUS, UNI_DESIGN_ID, NNI_CKT, C3A.STATUS NNI_STATUS, NNI_DESIGN_ID,
       CASE WHEN NL1.LOCATION_NAME LIKE '%NID%' OR NL1.LOCATION_NAME LIKE '%RAD %' OR NL1.LOCATION_NAME LIKE '%CIENA%' OR NL1.LOCATION_NAME LIKE '%CANOGA%' THEN 'YES' ELSE 'NO' END UNI_NID,
       NL1.LOCATION_ID UNI_LOC_ID, NL1.CLLI_CODE UNI_CLLI, NL1.LOCATION_NAME UNI_LOC, NL1.H_COORDINATE UNI_H_COORD, NL1.V_COORDINATE UNI_V_COORD,
       CASE WHEN NL2.LOCATION_NAME LIKE '%NID%' OR NL2.LOCATION_NAME LIKE '%RAD %' OR NL2.LOCATION_NAME LIKE '%CIENA%' OR NL2.LOCATION_NAME LIKE '%CANOGA%' THEN 'YES' ELSE 'NO' END UNI_NID2,
       NL2.LOCATION_ID UNI_LOC_ID2, NL2.CLLI_CODE UNI_CLLI2, NL2.LOCATION_NAME UNI_LOC2, NL2.H_COORDINATE UNI_H_COORD2, NL2.V_COORDINATE UNI_V_COORD2,
       CASE WHEN NL3.LOCATION_NAME LIKE '%NID%' OR NL3.LOCATION_NAME LIKE '%RAD %' OR NL3.LOCATION_NAME LIKE '%CIENA%' OR NL3.LOCATION_NAME LIKE '%CANOGA%' THEN 'YES' ELSE 'NO' END NNI_NID,
       NL3.LOCATION_ID NNI_LOC_ID, NL3.CLLI_CODE NNI_CLLI, NL3.LOCATION_NAME NNI_LOC, NL3.H_COORDINATE NNI_H_COORD, NL3.V_COORDINATE NNI_V_COORD,
       CASE WHEN NL4.LOCATION_NAME LIKE '%NID%' OR NL4.LOCATION_NAME LIKE '%RAD %' OR NL4.LOCATION_NAME LIKE '%CIENA%' OR NL4.LOCATION_NAME LIKE '%CANOGA%' THEN 'YES' ELSE 'NO' END NNI_NID2, 
       NL4.LOCATION_ID NNI_LOC_ID2, NL4.CLLI_CODE NNI_CLLI2, NL4.LOCATION_NAME NNI_LOC2, NL4.H_COORDINATE NNI_H_COORD2, NL4.V_COORDINATE NNI_V_COORD2,
       CCV1.CA_VALUE UNI_IP, CCV3.CA_VALUE NNI_IP, CCV4.CA_VALUE NNI_COPPER_IP, ACCT.PRIMARY_CARRIER_NAME CUSTOMER, CO2.LATA_NUMBER
FROM (
--4 
SELECT CKT EVC_CKT, EVC_DESIGN_ID, ACNA, BAN, B.DOCUMENT_NUMBER, EVC_IND, ACT, DD_COMP, UNI_SPEC, 
       CASE WHEN PNUM IS NOT NULL THEN PNUM ELSE OEC_PNUM END PNUM, LOS, EVC_SPEC, B.HBE_CIRCUIT,
       LATA, ADDRESS, CITY, STATE, EVC_CKR, EVC_BDW, EVC_LEVEL, EVC_STATUS, EVC_STATUS_DESC, ICSC,
...
FROM (
-- 2
SELECT DISTINCT CKT, ACNA, DOCUMENT_NUMBER, BAN, EVC_IND, ACT, DD_COMP, UNI_SPEC, PNUM, OEC_PNUM, LOS, EVC_SPEC,
       CASE WHEN LATA IS NOT NULL THEN LATA ELSE LATA_NUMBER END LATA,
       ADDRESS, CITY, STATE, EVC_CKR, ICSC,
...
WHERE SR.DOCUMENT_NUMBER = ASR.DOCUMENT_NUMBER
  ...
  --AND SR.DOCUMENT_NUMBER in ('3793406')
  AND TO_CHAR(T.ACTUAL_COMPLETION_DATE,'yyyymmdd') >= '20240114'     --CHANGE DATE TO SHOW THE PREVIOUS SUNDAY  
...
--RUN THIS FOR PENDING DISCONNECTS!!!!!!   
select *
from (
select asr.document_number,
       max(asr.pon) keep (dense_rank last order by asr.last_modified_date) pon, 
	   ...
	   MAX(ASR.NETWORK_CHANNEL_SERVICE_CODE) keep (dense_rank last order by asr.last_modified_date) NC,
	   max(asr.supplement_type) keep (dense_rank last order by asr.last_modified_date) supp,
       RTRIM(REPLACE(REPLACE(C.EXCHANGE_CARRIER_CIRCUIT_ID,'/','.'),' ',''),'.') CKT,
       max(asr.evc_ind) keep (dense_rank last order by asr.last_modified_date) evc_ind
from access_service_request asr,
	 serv_req sr,
	 network_location nl1,
