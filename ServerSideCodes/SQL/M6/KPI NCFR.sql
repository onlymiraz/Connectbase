...
SELECT DOCUMENT_NUMBER, PON, ACT_IND, COMP_DT, DD_TASK_COMP, ACNA, CUSTOMER,
 REGION, STATE, PRODUCT, CKT, 
 ...
 CASE WHEN MON = '12' THEN 'Jan'
      WHEN MON = '01' THEN 'Feb'
      WHEN MON = '02' THEN 'Mar'
      WHEN MON = '03' THEN 'Apr'
      WHEN MON = '04' THEN 'May'
      WHEN MON = '05' THEN 'Jun'
      WHEN MON = '06' THEN 'Jul'
      WHEN MON = '07' THEN 'Aug'
      WHEN MON = '08' THEN 'Sep'
      WHEN MON = '09' THEN 'Oct'
      WHEN MON = '10' THEN 'Nov'
      WHEN MON = '11' THEN 'Dec'
      ELSE NULL END MON,                                               --USE PLUS 1 TO ACCOMODATE THE SPREADSHEET WITH THIS ONE MONTH IN ARREARS
 CASE WHEN MON = '12' THEN YEAR+1
            ELSE YEAR END YEAR                   
 FROM (
    SELECT DISTINCT DOCUMENT_NUMBER, PON, ACT_IND, COMP_DT, DD_TASK_COMP, NPANXX, CLLI, ICSC, ACNA, CUSTOMER,
    REGION, TERRITORY, DIRECTOR, STATE, PRODUCT, CKT, TICKET_ID, ASSIGNMENTPROFILE, REPAIR_CODE, TFR.DISP, 
    TO_CHAR(DD_TASK_COMP,'MM') MON, EXTRACT(YEAR FROM DD_TASK_COMP) YEAR,TRBL_CREATE_DATE, TRBL_CLEARED_DT, 
    ...
    FROM (
        ...
        FROM ( 
            ...
            AND ACT_IND = 'N'
            AND TO_CHAR(DD_TASK_COMP,'YYYYMM') = '202308' --ONE MONTH IN ARREARS FROM REPORTING MONTH - MAKE SURE TO CHANGE EVERY MONTH   
     ...
     AND TO_CHAR(FLD_STARTDATE,'yyyymm') IN ('202308','202309')  --MUST CHANGE THESE DATES EACH MONTH TO SHOW PREVIOUS AND CURRENT MONTH  
...
SELECT document_number, ckt, act_ind, state, acna, customer, product, dd_task_comp, 
       CASE WHEN MON = '12' THEN 'Jan'
            WHEN MON = '01' THEN 'Feb'
            WHEN MON = '02' THEN 'Mar'
            WHEN MON = '03' THEN 'Apr'
       ...
       CASE WHEN MON = '12' THEN YEAR+1
            ELSE YEAR END YEAR
FROM (           
            SELECT DOCUMENT_NUMBER, DD_TASK_COMP, NCFR.NPANXX, NCFR.CLLI, ACNA, CUSTOMER, act_ind, ncfr.state, 
            PRODUCT, ckt,
            TO_CHAR(DD_TASK_COMP,'mm') MON,
            EXTRACT(YEAR FROM DD_TASK_COMP) YEAR,
            ...
            AND NCFR.CLLI = DIRC.CLLI (+) 
            AND ACT_IND = 'N'
  ) WHERE DD_TASK = '202308'                     --CHANGE THIS EACH MONTH  - SHOULD SHOW PREVIOUS MONTH     
    and product like 'ETHERNET%'
  ORDER BY 8,1,2,3;          
