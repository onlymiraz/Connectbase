select ticket_id, state, territory, region, clec_id, customer, ckt_id, product, create_date, cleared_dt, closed_dt, 
       ttr, repair_code, disp, site_clli, assignmentprofile profile, location_name 
from (
--
...
where a.exchange_carrier_circuit_id = e.EXCHANGE_CARRIER_CIRCUIT_ID(+) 
 ...
 and e.status (+) = '6'
 and (to_char(dte_closeddatetime,'yyyymm') in ('202003')    --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS)
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202003')))   --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS) 
