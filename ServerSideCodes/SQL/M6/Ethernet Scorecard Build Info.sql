SELECT DISTINCT DOCUMENT_NUMBER, INIT, 
       ...
       BUILD_IOF, BUILD_OSP,
       PNUM, 
       CASE WHEN ETH_PROD IS NULL THEN 'OTHER' ELSE ETH_PROD END ETH_PROD
FROM (
SELECT DISTINCT DOCUMENT_NUMBER, TRUNC(ASR_INIT) INIT,  
       (SELECT UDCV.DISPLAY_VALUE FROM USER_DATA_CATEGORY_VALUES UDCV WHERE UNI_OR_NNI = UDCV.USER_DATA_CATEGORY_VALUE_ID) AS UNI_OR_NNI,      
       (SELECT UDCV.DISPLAY_VALUE FROM USER_DATA_CATEGORY_VALUES UDCV WHERE BUILD_IOF = UDCV.USER_DATA_CATEGORY_VALUE_ID) AS BUILD_IOF,
       (SELECT UDCV.DISPLAY_VALUE FROM USER_DATA_CATEGORY_VALUES UDCV WHERE BUILD_OSP = UDCV.USER_DATA_CATEGORY_VALUE_ID) AS BUILD_OSP,      
       PNUM, 
       MAX(ETH_PROD) ETH_PROD
FROM (
SELECT SR.DOCUMENT_NUMBER, 
       ...
       MIN(N.LAST_MODIFIED_DATE) KEEP (DENSE_RANK FIRST ORDER BY N.LAST_MODIFIED_DATE) ASR_INIT,
...
FROM ASR_USER_DATA AUD, 
     ...
     ACCESS_BILLING_CKT_LOC_USOCS ABCU,
     CIRCUIT CIR,
...
WHERE SR.DOCUMENT_NUMBER = ASR.DOCUMENT_NUMBER
  ...
  and ABCD.CABS_CIRCUIT_ID = ABCU.CABS_CIRCUIT_ID (+)
  AND SR.FIRST_ECCKT_ID = CIR.EXCHANGE_CARRIER_CIRCUIT_ID (+)
  AND CIR.CIRCUIT_DESIGN_ID = CUD.CIRCUIT_DESIGN_ID (+)
  AND TO_CHAR(T.ACTUAL_COMPLETION_DATE,'yyyymm') = '202308'   --CHANGE EACH MONTH - CURRENT REPORTING MONTH  
  AND ASR.REQUEST_TYPE IN ('S','E')
  AND ASR.ACTIVITY_INDICATOR IN ('N','C')
  AND ASR.ORDER_TYPE = 'ASR'
  AND T.TASK_TYPE = 'DD'
  --AND SR.DOCUMENT_NUMBER = '3271136'
  GROUP BY SR.DOCUMENT_NUMBER, ABCU.USOC_CODE, UNI_OR_NNI
)  
  WHERE (NC NOT IN ('TD','TR') OR NC IS NULL)
...
GROUP BY DOCUMENT_NUMBER, ASR_INIT, BUILD_IOF, BUILD_OSP, PNUM, UNI_OR_NNI  
)                 
ORDER BY 1;                    
