drop table kpitrouble
/


create table kpitrouble nologging nocache as
select ticket_id, state, region, territory, director, clec_id, customer, ckt_id, product, create_date, cleared_dt, closed_dt, 
       ttr, repair_code, disp, site_clli, assignmentprofile profile, location_name, clli_code1 clli_code, to_char(closed_dt,'MM') mon 
from (
--
select ticket_id, data2.state,  
       ...
       repair_code, disp, clli_code site_clli,
       reqstat, location_name, assignmentprofile, clli_code1
from (
--
...
select distinct ticket_id, 
       ...
       Total_Duration, TTR, a1.repair_code, b.disp, reqstat, clli_code1,
...
select ticket_id, site_clli6, site_state, ckt_id, circuit, a.acna, request_type, create_date,
       ...
       assignmentprofile, service_type_code, rate_code,
...
where a.exchange_carrier_circuit_id = e.EXCHANGE_CARRIER_CIRCUIT_ID(+) 
 ...
 and e.status (+) = '6'
 and (to_char(dte_closeddatetime,'yyyymm') in ('202308','202309')    --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS)
    or (dte_closeddatetime is null and to_char(fld_event_end_time,'yyyymm') in ('202308','202309')))   --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS) 
...
select ticket_id, state, region, clec_id, customer, ckt_id, product, create_date, cleared_dt, closed_dt, 
       ttr, repair_code, disp, site_clli,
       CASE WHEN TO_CHAR(CLOSED_DT,'MM') = '01' THEN 'Jan'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '02' THEN 'Feb'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '03' THEN 'Mar'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '04' THEN 'Apr'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '05' THEN 'May'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '06' THEN 'Jun'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '07' THEN 'Jul'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '08' THEN 'Aug'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '09' THEN 'Sep'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '10' THEN 'Oct'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '11' THEN 'Nov'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '12' THEN 'Dec'
            ELSE NULL END MON,
       EXTRACT(YEAR FROM CLOSED_DT) YEAR,
       case when disp in ('FAC','CO','NTF','CC') then 'Yes' else 'No' end "TTR Count"     
  from kpitrouble
  where product = 'Ethernet'
  and mon = '09'
  order by 10;     


--FOR REPEAT DETAIL, RUN THROUGH THESE STEPS     

drop table kpirepeat
/

create table kpirepeat nologging nocache as
select * 
from kpitrouble
where mon = '09'   --NEED TO CHANGE THIS EACH MONTH
and disp in ('CO','FAC')
and product = 'Ethernet';



select ticket_id, state, region, clec_id, customer, product,
       ckt_id, create_date, cleared_dt, closed_dt, disp, prev_ticket_id, prev_create_dt, prev_clear_dt,
       prev_disp, round(create_date-prev_clear_dt,0) days, 
       CASE WHEN TO_CHAR(CLOSED_DT,'MM') = '01' THEN 'Jan'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '02' THEN 'Feb'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '03' THEN 'Mar'
            WHEN TO_CHAR(CLOSED_DT,'MM') = '04' THEN 'Apr'
       ...
       EXTRACT(YEAR FROM CLOSED_DT) YEAR      
from (
select r.ticket_id, r.state, r.region, r.territory, r.director, r.site_clli, r.clec_id, r.customer,
       ...
       max(t.disp) keep (dense_rank last order by t.create_date) prev_disp
from kpirepeat r, kpitrouble t 
where r.CKT_ID = t.ckt_id
and r.CREATE_DATE > t.CLEARED_DT
and r.ticket_id <> t.ticket_id
and t.disp in ('CO','FAC','NTF','CC')
group by r.ticket_id, r.state, r.region, r.territory, r.director, r.site_clli, r.clec_id, r.customer,
         r.product, r.ckt_id, r.create_date, r.cleared_dt, r.closed_dt, r.disp
)
