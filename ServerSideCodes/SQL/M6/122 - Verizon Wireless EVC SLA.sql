...
'Carrier CTF' PRODUCT, HBE_CIRCUIT,
...
NID_TYPE,
CASE WHEN NID_TYPE IN ('RAD','CANOGA') AND CCV5.CA_VALUE IS NOT NULL AND CCV5.CA_VALUE <> '0' THEN REPLACE(CCV5.CA_VALUE,' ','')
     WHEN NID_TYPE IN ('RAD','CANOGA') AND CCV6.CA_VALUE IS NOT NULL AND CCV6.CA_VALUE <> '0' THEN REPLACE(CCV6.CA_VALUE,' ','')
     WHEN NID_TYPE = 'CIENA' AND CCV7.CA_VALUE IS NOT NULL AND CCV7.CA_VALUE <> '0' THEN REPLACE(CCV7.CA_VALUE,' ','')
     WHEN NID_TYPE = 'CIENA' AND CCV8.CA_VALUE IS NOT NULL AND CCV8.CA_VALUE <> '0' THEN REPLACE(CCV8.CA_VALUE,' ','')
     ELSE NULL END VLAN       
FROM (
-- 7 
SELECT BAN, ACNA, CUSTOMER, 
REPLACE(ADDRESS,',','') ADDRESS, REPLACE(CITY,',','') CITY,  REPLACE(STATE,',','') STATE, 
LATA, PNUM, DD_COMP, DOCUMENT_NUMBER, ACT, EVC_BDW, LOS, EVC_SPEC, EVC_LEVEL, EVC_CKR, HBE_CIRCUIT,
...
SELECT EVC_CKT, EVC_DESIGN_ID, ACNA, BAN, A.DOCUMENT_NUMBER, EVC_IND, ACT, DD_COMP, UNI_SPEC, PNUM, LOS, EVC_SPEC,
       LATA, ADDRESS, CITY, STATE, EVC_BDW, EVC_LEVEL, EVC_STATUS, EVC_STATUS_DESC, EVC_CKR, HBE_CIRCUIT,
...
SELECT CKT EVC_CKT, EVC_DESIGN_ID, ACNA, BAN, B.DOCUMENT_NUMBER, EVC_IND, ACT, DD_COMP, UNI_SPEC, PNUM, LOS, EVC_SPEC,
       LATA, ADDRESS, CITY, STATE, EVC_BDW, EVC_LEVEL, EVC_STATUS, EVC_STATUS_DESC, EVC_CKR, B.HBE_CIRCUIT,
...
SELECT DISTINCT C.EXCHANGE_CARRIER_CIRCUIT_ID CKT, SR.ACNA, SR.DOCUMENT_NUMBER, EVC_IND, TRUNC(ACTUAL_COMPLETION_DATE) DD_COMP,
       ...
       ED.EVC_CKR, AUD.BAN, C2.STATUS EVC_STATUS, C.CIRCUIT_DESIGN_ID, CUDEVC.HBE_CIRCUIT,
       DECODE (C2.STATUS,
                  '1', 'Pending',
                  '2', 'Assigned',
                  '3', 'In Progress',
...
WHERE SR.DOCUMENT_NUMBER = ASR.DOCUMENT_NUMBER
  ...
  --AND SR.DOCUMENT_NUMBER in ('3299517','3299526')
  AND TO_CHAR(T.ACTUAL_COMPLETION_DATE,'yyyymmdd') >= '20240114'     --CHANGE DATE TO SHOW THE PREVIOUS SUNDAY  
  AND PROMOTION_NBR IN ('SEWVZWFLAT850','SEWVZWFLAT1250')
  AND SR.ACNA in ('BLG','CCE','CCQ','CEL','CEX','CGO','CKZ','AAK','AHG','AKR','AOK','AUL','BAM','BBK','FCO','FCS','IAP','ICN','IDE','IPD',
