SELECT TICKET_ID, CREATE_DATE, CLEARED_DT, CLOSED_DT, CKT_ID, 
       REPLACE(CIRCUIT,' ','') CIRCUIT, STATE, TTR, 
       TRUNC(((((30*24)-(TTR))/(30*24))*100),3) CIRCUIT_AVAIL,  --MUST CHANGE THE # OF DAYS FOR THIS FORMULA EACH MONTH 
       REQUEST_TYPE, REPAIR_CODE, DISP,
       CASE WHEN REQUEST_TYPE = 'Information' THEN 'No'
            WHEN DISP IN ('CPE','REF','EXC','NTF') THEN 'No'
            ELSE 'Yes' END "Include in S/R EVC Avail",
       CASE WHEN REQUEST_TYPE = 'Information' THEN 'No'
            WHEN DISP IN ('CPE','REF','EXC','NTF','CC') THEN 'No'
            ELSE NULL END "Include in MTTR",
...
SELECT DISTINCT TICKET_ID, 
       CASE WHEN SITE_STATE IS NOT NULL AND SITE_STATE IN ('OR','WA','ID','MT') THEN SITE_STATE
            WHEN PRILOC_STATE IS NOT NULL AND PRILOC_STATE IN ('OR','WA','ID','MT') THEN PRILOC_STATE
            ELSE CLLI_STATE END STATE,
       CASE WHEN CCNA1 IN ('AWL','IUW') THEN CCNA1
            WHEN ACNA3 IN ('AWL','IUW') THEN ACNA3
            WHEN CCNA2 IN ('AWL','IUW') THEN CCNA2
            WHEN ACNA1 IN ('AWL','IUW') THEN ACNA1
            WHEN ACNA2 IN ('AWL','IUW') THEN ACNA2
            WHEN ACNA2 IN ('AWL','IUW') THEN ACNA2
...
SELECT TICKET_ID, SITE_CLLI6, SITE_STATE, CKT_ID, CIRCUIT, A.ACNA, REQUEST_TYPE, CREATE_DATE,
       ...
       SERVICE_TYPE_CODE, RATE_CODE, TROUBLE_DESC, PROFILE, TRBLSTAT, 
       ...
       MAX(TRIM(D2.CLEC_ID)) KEEP (DENSE_RANK FIRST ORDER BY D2.CLEC_ID) ACNA3,
...
WHERE A.EXCHANGE_CARRIER_CIRCUIT_ID = E.EXCHANGE_CARRIER_CIRCUIT_ID(+) 
 ...
 AND E.STATUS (+) = '6'
 AND (TO_CHAR(DTE_CLOSEDDATETIME,'yyyymm') = '202009'                                      --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS)
    OR (DTE_CLOSEDDATETIME IS NULL AND TO_CHAR(FLD_EVENT_END_TIME,'yyyymm') = '202009'))   --NEED TO CHANGE THIS EACH MONTH (CURRENT AND PREVIOUS MONTHS) 
...
-- 1 
 ...
 GROUP BY TICKET_ID, SITE_CLLI6, SITE_STATE, CKT_ID, CIRCUIT, A.ACNA, REQUEST_TYPE, CREATE_DATE,
          ...
          PROFILE, SERVICE_TYPE_CODE, RATE_CODE, TROUBLE_DESC
-- 2 
)
