...
select 'Non-Loop' product, tariff_code, sum(num) met, sum(den) orders, round((sum(num)/sum(den)*100),2) "% Met", sum(backlog) past_due
from ( 
select distinct pon, tariff_code,
       1 num, null den, null backlog
FROM CAMP.STG_SERVICE_ORDERS_LSR DPI 
where to_char(order_compl_dt,'yyyymmdd') = '20160428'  -- change to previous workday    
and (ccna not in ('FTR') 
...
where to_char(dd,'yyyymmdd') <= '20160428'
and comp is null 
