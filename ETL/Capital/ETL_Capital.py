...
node_db_mapping = {
    'WADINFWWAPV02': 'WAD_PRD_Capital',
    'WADINFWWDDV01': 'WAD_STG_Capital'
    # Add other nodes and their corresponding databases here
}

# Get the current node
current_node = socket.gethostname().upper()
database_name = node_db_mapping.get(current_node, 'WAD_STG_Capital')  # Default to 'Playground' if node not found
...
def find_column_name(table_name):
    ...
    wad_stg_capital_conn_str = (
        f"Driver={{ODBC Driver 17 for SQL Server}};"
        f"Server={current_node};"
        f"Database=WAD_STG_Capital;"
        "Trusted_Connection=yes;"
    )
    
    try:
        wad_stg_capital_conn = pyodbc.connect(wad_stg_capital_conn_str)
        wad_stg_capital_cursor = wad_stg_capital_conn.cursor()
        
        # Query to retrieve column names for the table from any schema except LZ_Capital
        wad_stg_capital_cursor.execute(f"""
            SELECT TABLE_SCHEMA, COLUMN_NAME
            FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_NAME = ?
              AND TABLE_SCHEMA <> 'LZ_Capital'
        """, table_name)
        
        columns = [row.COLUMN_NAME for row in wad_stg_capital_cursor.fetchall()]
        
        # If no columns are found, log a warning and return an empty list
        if not columns:
            logging.warning(f"No columns found for table {table_name} in WAD_STG_Capital")
            print(f"No columns found for table {table_name} in WAD_STG_Capital")
...
def copy_files_to_output_directory(src_directory, dest_directory, lzFiles, ):
    copied_files = []
    
    # List all files in the source directory (not recursively)
    for file in os.listdir(src_directory):

        # if file is not in lzFiles.items with csv appended to the end, skip
        if file not in [lz_value + ".csv" for lz_values in lzFiles.values() for lz_value in lz_values]:
            continue
        ...
        if os.path.isfile(file_path) and file.lower().endswith('.csv'):
            ...
            print(f"Skipped non-CSV file: {file}")
...
def rename_files(source_directory, lzFiles):
    """
    Rename files in the source directory according to the lzFiles.
    """

    for lz_key, lz_values in lzFiles.items():
        for lz_value in lz_values:
            old_file_path = os.path.join(source_directory, lz_value + ".csv")
            new_file_path = os.path.join(source_directory, lz_key + ".csv")
...
copied_files = copy_files_to_output_directory(source_directory, output_directory, lzFiles)

# rename files
rename_files(output_directory, lzFiles)
