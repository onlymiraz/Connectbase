...
oracle_pw = 'asap'
teradata_pw = os.getenv('TERADATA_PW')

# Create a DSN (Data Source Name) for Oracle
dsn = cx_Oracle.makedsn("asapprdstdby.corp.pvt", 1521, service_name="asapprd")
oracle_conn_str = f"user='ASAP', password='{oracle_pw}', dsn={dsn}"

# Connection strings
teradata_conn_str = json.dumps({'host': '10.209.129.144', 'user': 's_WAD3', 'password': teradata_pw, 'logmech': 'TD2'})
