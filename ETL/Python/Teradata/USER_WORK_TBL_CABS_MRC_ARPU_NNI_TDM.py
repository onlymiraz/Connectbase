...
update_queries =["DELETE USER_WORK.TBL_CABS_MRC_ARPU_NNI_TDM;","""INSERT INTO USER_WORK.TBL_CABS_MRC_ARPU_NNI_TDM
SELECT DISTINCT
...
-- CASE WHEN SVC_GROUP = 'ETH_NNI' THEN PRODUCT ELSE
CASE WHEN SVC_GROUP = 'ETH_NNI' THEN PRODUCT
     WHEN SUBSTR(SVC_GROUP,1,5) = ('COLLO') THEN SVC_GROUP
     WHEN PRODUCT = 'DRK FBR - NON REG' THEN PRODUCT
     WHEN SVC_GROUP = 'DRK_FBR' THEN SVC_GROUP
     WHEN PRODUCT = 'UNKNOWN' AND SVC_GROUP IN ('TDM_DS1','TDM_DS3','OCN') THEN 'TDM/OCN' 
     WHEN PRODUCT IS NOT NULL THEN PRODUCT 
     ELSE '-' END AS PRODUCT,
STATE,     
SVC_GROUP,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1)-- AND WIRELESS = 'WIRELINE' 
  THEN TOTAL_MRC END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS MRC_ALL_BILLED,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1)-- AND WIRELESS = 'WIRELINE'
  THEN CMPY_NO_COUNT END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS COUNT_ALL_BILLED,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1)-- AND WIRELESS = 'WIRELINE'
  THEN TOTAL_MRC END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) /
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1)-- AND WIRELESS = 'WIRELINE'
  THEN CMPY_NO_COUNT END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS ARPU_ALL_BILLED,

SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND FIRST_BILL_MONTH_DT = BILL_MONTH_DT -- AND WIRELESS = 'WIRELINE' 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) 
  THEN TOTAL_MRC END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS MRC_FIRST_BILL,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND FIRST_BILL_MONTH_DT = BILL_MONTH_DT -- AND WIRELESS = 'WIRELINE'
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) 
  THEN CMPY_NO_COUNT END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS COUNT_FIRST_BILL,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND FIRST_BILL_MONTH_DT = BILL_MONTH_DT -- AND WIRELESS = 'WIRELINE'
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) 
  THEN TOTAL_MRC END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) /
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND FIRST_BILL_MONTH_DT = BILL_MONTH_DT -- AND WIRELESS = 'WIRELINE'
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) 
  THEN CMPY_NO_COUNT END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS ARPU_FIRST_BILL,
  
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND LAST_BILL_MONTH_DT = BILL_MONTH_DT
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) -- AND WIRELESS = 'WIRELINE' 
  THEN TOTAL_MRC END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS MRC_LAST_BILL,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND LAST_BILL_MONTH_DT = BILL_MONTH_DT
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) -- AND WIRELESS = 'WIRELINE'
  THEN CMPY_NO_COUNT END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS COUNT_LAST_BILL,
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND LAST_BILL_MONTH_DT = BILL_MONTH_DT 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) -- AND WIRELESS = 'WIRELINE'
  THEN TOTAL_MRC END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) /
SUM(CASE WHEN SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR') AND LAST_BILL_MONTH_DT = BILL_MONTH_DT 
  AND BILL_CYCLE_DT >= BILL_MONTH_DT AND BILL_CYCLE_DT < ADD_MONTHS(BILL_MONTH_DT,1) -- AND WIRELESS = 'WIRELINE'
  THEN CMPY_NO_COUNT END) OVER (PARTITION BY BILL_MONTH_DT,SVC_GROUP,WIRELESS,TOP_CUST,PRIMARY_CARRIER_NAME,PRODUCT,STATE) AS ARPU_LAST_BILL
FROM
-------------------------------------------------------------------------------------------------------------------------
( -- SUBQ3
SELECT DISTINCT
...
CASE WHEN (SUBQ2.STATE <> 'CT' AND SUBQ2.NNI = 'NNI') THEN 'ETH_NNI'
     WHEN (SUBQ2.STATE = 'CT' AND SUBSTR(SUBQ2.NC_PRODUCT_CD,1,2) IN ('KD','KF','KG','KJ','KP','KR','KS','KU','SN')
...
--NVL(SUBQ2.PRODUCT,'UNKNOWN') AS PRODUCT, 
NVL(SUBQ2.PRODUCT, 
...
WHERE CABS0.BILL_MONTH_DT BETWEEN ADD_MONTHS(CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE)+1, -16) 
 AND ADD_MONTHS(CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE)+1, -1)  
--  WHERE CABS0.BILL_MONTH_DT BETWEEN '2023-10-01' AND '2023-10-01' -- >= '2019-12-01') 
  AND (CABS0.SW_SPL_IND = 'SP' 
       OR SUBSTR(CABS0.NC_PRODUCT_CD,1,4) IN ('COLL','COLO') OR SUBSTR(CABS0.PLAN_ID,1,3) = 'DKF')
...
-- WHERE CABS.BILL_MONTH_DT BETWEEN '2023-10-01' AND '2023-10-01' -- SPECIFY BILL MONTH RANGE TO RETRIEVE
-- AND (SUBSTR(CABS.NC_PRODUCT_CD,1,1) = 'O' OR SUBSTR(CABS.NC_PRODUCT_CD,1,2) IN ('HF','HC'))
AND (SUBSTR(CABS.NC_PRODUCT_CD,1,1) IN ('K','O') OR SUBSTR(CABS.NC_PRODUCT_CD,1,2) IN ('HF','HC','VL','SN')
OR SUBSTR(CABS.NC_PRODUCT_CD,1,4) IN ('COLL','COLO') OR SUBSTR(CABS.PLAN_ID,1,3) = 'DKF')
...
WHERE SVC_GROUP IN ('ETH_NNI','TDM_DS1','TDM_DS3','OCN','COLLO_REG','COLLO_NONREG','DRK_FBR');"""]
