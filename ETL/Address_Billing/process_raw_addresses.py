...
def parse_addresses(df_l, df_r):
    ...
    # like basic but include predirection (ex: 123 "W" main st)
    ...
    # bring all match technique resulting DFs together
    df_usaddr_match = pd.concat(objs=[
        # df_usaddr_match_basic,  # probably should drop due to FPs
        df_usaddr_match_prdir,  # likely best one
        df_usaddr_match_compl,  # too strict, likely missing
        df_usaddr_match_onzip,  # supplemental, should improve results
        df_usaddr_match_nozip,  # supplemental, should improve results
    ],
        axis=0)

    # drop duplicate match results
    df_usaddr_match.drop_duplicates(subset=['index_l', 'index_r'],
                                    inplace=True)
