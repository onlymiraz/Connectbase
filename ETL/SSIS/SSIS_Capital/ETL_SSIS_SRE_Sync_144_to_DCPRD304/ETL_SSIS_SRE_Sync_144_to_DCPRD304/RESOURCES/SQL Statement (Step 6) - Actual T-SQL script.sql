...
--- EXAMPLE TABLE IN THIS SCRIPT: forecast.SummaryColumn ("+ @[User::TableName_str] +" in string expression for SSIS package)
...
EXEC('TRUNCATE TABLE CapitalManagementProduction.forecast.SummaryColumn') AT [DBFarm304]
...
SET @PK_Condition = (
	...
	WHERE [CONSTRAINT_TYPE] = 'PRIMARY KEY' AND TABLE_NAME = SUBSTRING('forecast.SummaryColumn', CHARINDEX('.','forecast.SummaryColumn')+1, 20000)
)
...
SET @ID_Condition = (
		...
		WHERE TAB.name = SUBSTRING('forecast.SummaryColumn', CHARINDEX('.','forecast.SummaryColumn')+1, 20000)
	)
...
SET @PK_Table_Columns = (REPLACE( (SELECT SUBSTRING(
			(
				...
				WHERE TABLE_NAME = SUBSTRING('forecast.SummaryColumn', CHARINDEX('.','forecast.SummaryColumn')+1, 20000) AND TABLE_SCHEMA = SUBSTRING('forecast.SummaryColumn', 0, CHARINDEX('.','forecast.SummaryColumn'))
...
SET @Main_Insert_Query = N'
	...
	INSERT INTO [DBFarm304].CapitalManagementProduction.forecast.SummaryColumn ('+ @PK_Table_Columns +') 
			SELECT ' + @PK_Table_Columns + ' FROM CAPINFWWAPV01.CapitalManagementProduction.forecast.SummaryColumn
'
...
'
	...
	-- NOTE: Some of the tables may have primary keys/identity columns on the source table, but will not have any on the destination table
		-- So checks to see if the specified DESTINATION table has an primary keys/identity columns, then sets IDENTITY_INSERT constraint based on condition
...
SET @Main_Insert_Query_PK2 = N'
	...
	IF(' + CAST(@ID_Condition AS nvarchar) + ' > 0)
		BEGIN
			SET IDENTITY_INSERT CapitalManagementProduction.forecast.SummaryColumn ON

			INSERT INTO CapitalManagementProduction.forecast.SummaryColumn ('+ @PK_Table_Columns +') 
					SELECT ' + @PK_Table_Columns + ' FROM CapitalManagementProduction.dbo.TempDestinationTable

			SET IDENTITY_INSERT CapitalManagementProduction.forecast.SummaryColumn OFF
		END
...
IF(@ID_Condition > 0 OR @PK_Condition > 0)
	BEGIN
		EXECUTE [DBFarm304].CapitalManagementProduction.dbo.createtemptable 'forecast.SummaryColumn'
...
--SELECT * FROM forecast.SummaryColumn
---CONFIRMING INSERTS
--select * from CAPINFWWAPV01.CapitalManagementProduction.forecast.SummaryColumn
--select * from [DBFarm304].CapitalManagementProduction.forecast.SummaryColumn
--TRUNCATE TABLE [DBFarm304].CapitalManagementProduction.forecast.SummaryColumn